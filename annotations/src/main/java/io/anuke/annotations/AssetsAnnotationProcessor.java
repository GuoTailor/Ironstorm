...
public class AssetsAnnotationProcessor extends AbstractProcessor{
    ...
    public boolean process(Set<? extends TypeElement> annotations, RoundEnvironment roundEnv){
        ...
        try{
            ...
            processSounds("Sounds", path + "/assets/sounds", "com.badlogic.gdx.audio.Sound");
            processSounds("Musics", path + "/assets/music", "com.badlogic.gdx.audio.Music");
...
//            filename = filename.replace(".9.png", "").replace(".png", "");
            ...
            for(String suffix : names){
                ...
                String dtype = p.getFileName().toString().endsWith(".9.png") ? "com.badlogic.gdx.scenes.scene2d.utils.NinePatchDrawable" : "com.badlogic.gdx.scenes.scene2d.utils.TextureRegionDrawable";
                ...
                tload.addStatement(varname + " = ("+dtype+")com.phoenix.game.core.Core.skin.getDrawable($S)", sfilen);
            }
        });
        ...
        for(Element elem : elements){
            ...
            t.getEnclosedElements().stream().filter(e -> e.getKind() == ElementKind.FIELD).forEach(field -> {
                ...
                if(fname.startsWith("default")){
                    //loadStyles.addStatement("com.phoenix.game.Core.scene.addStyle(" + field.asType().toString() + ".class, io.anuke.mindustry.ui.Styles." + fname + ")");
                }
            });
        }
    ...
    void processSounds(String classname, String path, String rtype) throws Exception{
        ...
        MethodSpec.Builder updateBegin = MethodSpec.methodBuilder("update").addModifiers(Modifier.PUBLIC, Modifier.STATIC);
        ...
        Files.list(Paths.get(path)).forEach(p -> {
            ...
            String filename = "com.phoenix.game.core.Core.app.getType() != com.badlogic.gdx.Application.ApplicationType.iOS ? \"" + filepath + "\" : \"" + filepath.replace(".ogg", ".mp3")+"\"";

            loadBegin.addStatement("com.phoenix.game.core.Core.assets.load("+filename +", "+rtype+".class)");

            updateBegin.addStatement(name + " = com.phoenix.game.core.Core.assets.get(" + filename + ", com.badlogic.gdx.backends.lwjgl3.audio.mock.Mock" + rtype.substring(rtype.lastIndexOf(".") + 1) + ".class)");
            dispose.addStatement(name + ".dispose()");
            dispose.addStatement(name + " = null");
            type.addField(FieldSpec.builder(ClassName.bestGuess(rtype), name, Modifier.STATIC, Modifier.PUBLIC).initializer("new com.badlogic.gdx.backends.lwjgl3.audio.mock.Mock" + rtype.substring(rtype.lastIndexOf(".") + 1)+ "()").build());
        });

        if(classname.equals("Sounds")){
            type.addField(FieldSpec.builder(ClassName.bestGuess(rtype), "none", Modifier.STATIC, Modifier.PUBLIC).initializer("new com.badlogic.gdx.backends.lwjgl3.audio.mock.Mock" + rtype.substring(rtype.lastIndexOf(".") + 1)+ "()").build());
        }

        type.addMethod(loadBegin.build());
        type.addMethod(updateBegin.build());
